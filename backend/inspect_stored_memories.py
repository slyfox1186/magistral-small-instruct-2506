...
def inspect_memory_database():
    ...
    try:
        ...
        if count > 0:
            ...
            logger.info(f"\n{'='*60}")
            logger.info(f"SEARCHING FOR ANIMAL/SOUL RELATED MEMORIES")
            logger.info(f"{'='*60}")
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"RECENT MEMORIES (LAST 10)")
        logger.info(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"TESTING MEMORY SEARCH FUNCTIONALITY")
    logger.info(f"{'='*80}")
