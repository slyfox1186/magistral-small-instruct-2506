...
def extract_personal_info_patterns(text: str) -> dict[str, str]:
    ...
    for pattern in job_patterns:
        ...
        if match:
            ...
            if (
                job not in ["good", "fine", "ok", "here", "there", "new", "old"]
                and len(job) > MIN_JOB_NAME_LENGTH
            ):
                info["job"] = job
                break
...
def validate_memory_content(
    content: str, min_length: int = 10, max_length: int = 2000
) -> tuple[bool, str]:
...
def create_memory_summary(memories: list[dict[str, Any]], max_length: int = 200) -> str:
    ...
    for memory in memories:
        ...
        if content:
            ...
            topics.extend([word for word in words if len(word) > MIN_WORD_LENGTH_FOR_TOPICS])
...
def log_memory_metrics(operation: str, duration: float, success: bool, **kwargs):
    ...
    logger.info(
        f"[MEMORY_METRICS] {operation} {status} in {duration:.2f}s",
        extra={
            "operation": operation,
            "duration": duration,
            "success": success,
