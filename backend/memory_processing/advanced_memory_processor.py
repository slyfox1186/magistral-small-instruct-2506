...
class AdvancedMemoryProcessor:
    ...
    async def _validate_storage(self, memories: List[ExtractedMemory], session_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error validating storage: {str(e)}")
            return False
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    self.logger.error(f"Memory processing error (attempt {attempt + 1}/{max_retries + 1}): {str(e)}, "
                                    f"retrying in {wait_time:.1f}s...")
                    await asyncio.sleep(wait_time)
                else:
                    ...
                    return MemoryProcessingResult(
                        success=False,
                        error_message=f"Memory processing failed after {max_retries + 1} attempts: {str(e)}",
                        memories_stored=0,
                        processing_time=0.0,
                        stage_timings={},
                        content_analysis=None,
