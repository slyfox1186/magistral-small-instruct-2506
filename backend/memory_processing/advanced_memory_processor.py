...
HEALTHY_SUCCESS_RATE = 90
DEGRADED_SUCCESS_RATE = 70
...
class AdvancedMemoryProcessor:
    ...
    def __init__(self, memory_system, config: MemoryProcessingConfig):
        """Initialize the processor with memory system and configuration.

        Args:
            memory_system: The memory system instance for storage
        ...
        try:
            ...
            if not content_analysis.memory_worthy:
                ...
                return self._create_empty_result(
                    session_id, time.time() - start_time, stage_timings
                )
            ...
            if self.config.enable_detailed_logging:
                self.logger.info(
                    f"Memory processing complete for session {session_id}: "
    ...
    async def _store_memories(self, memories: list[ExtractedMemory], session_id: str) -> int:
        ...
        for memory in memories:
            try:
                ...
                if self.config.enable_detailed_logging:
                    self.logger.debug(
                        f"Stored {memory.memory_type} memory: {memory.content[:50]}..."
                    )

            except Exception:
                self.logger.exception("Error storing memory")
                continue
    ...
    async def _validate_storage(self, memories: list[ExtractedMemory], session_id: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"Validation check: {len(memories)} memories to validate, {len(stored_contents)} stored contents"
            )
        except Exception:
            self.logger.exception("Error validating storage")
            return False
        else:
            return True  # Validation success based on storage logs

    def _create_empty_result(
        self, session_id: str, processing_time: float, stage_timings: dict[str, float]
    ) -> MemoryProcessingResult:
    ...
    def _update_processing_stats(self, processing_time: float, memories_stored: int, success: bool):
        ...
        for attempt in range(max_retries + 1):
            try:
                result = await self.process_conversation(
                    user_prompt, assistant_response, session_id
                )
                ...
                if attempt < max_retries:
                    ...
                    self.logger.warning(
                        f"Memory processing failed (attempt {attempt + 1}/{max_retries + 1}), "
                ...
                if attempt < max_retries:
                    ...
                    self.logger.exception(
                        f"Memory processing error (attempt {attempt + 1}/{max_retries + 1}), "
                        f"retrying in {wait_time:.1f}s..."
                    )
                    await asyncio.sleep(wait_time)
                else:
